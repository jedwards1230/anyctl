...
// rest; a loose file mode (group/world readable or writable) would defeat
// that boundary for any other local account, so it is refused. Returns:
...
// checkTokenFilePerms refuses to use a service-account token file that is
// readable or writable by group or world (mode & 0077 != 0). This token is a
// 1Password service-account credential with broad blast radius if leaked —
// treated like ssh treats a loose private-key permission, not policy logic:
// same category as refusing a corrupt config file. Returns *AuthError (the
// same family as the other file-source failures in token()) naming the exact
// bits at fault and the fix.
func checkTokenFilePerms(path string) error {
	...
	if mode := info.Mode().Perm(); mode&0o077 != 0 {
		return &AuthError{Err: fmt.Errorf(
			"service_account_token: file %s is readable/writable by group or world (mode %#o); fix with: chmod 0600 %s",
			path, mode, path,
		)}
	}
	return nil
