...
func cacheDir() string {
	if d := os.Getenv("XDG_CACHE_HOME"); d != "" {
		return filepath.Join(d, brand.Name)
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return filepath.Join(".cache", brand.Name)
	}
	return filepath.Join(home, ".cache", brand.Name)
}
...
func fetchOAuth2Token(ctx context.Context, a manifest.Auth, env template.Env, dir string) (string, error) {
	...
	if err := writeCache(cachePath, result.AccessToken, result.ExpiresIn); err != nil {
		...
		_, _ = fmt.Fprintf(os.Stderr, "%s: oauth2 cache write: %v\n", brand.Name, err)
	}
