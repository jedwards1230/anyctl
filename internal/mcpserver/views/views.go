...
// npm. ANYCTL_VIEWS_DIR overrides the embedded copy with a live file from disk
// — mirrors ANYCTL_CONFIG_DIR — for iterating on views/ without a Go rebuild.
package views
...
// ResultMIMEType is the MIME type advertised for the ui://anyctl/result
...
// ANYCTL_VIEWS_DIR/result.html when that env var is set and the file is
// readable, otherwise the embedded copy. Read once per call so a server rebuilt
// with the override set always picks up the latest build on disk without a Go
// rebuild; BuildServer calls this once at server-construction time, matching
